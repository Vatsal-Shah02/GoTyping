...
class SpeedTest {
    ...
    displayWPM() {
        ...
        if (charIndex < characters.length - 1 && timeLeft > 0) {
            ...
            let accuracy=Math.floor(((charIndex-error)/charIndex)*100);
            wpmTag.innerText = wpm;
            mistakeTag.innerText = mistakes;
            cpmTag.innerText = accuracy+'%';
        } else {
            clearInterval(timer);
            inpField.value = "";
        }
...
speedTest.getPara();
inpField.addEventListener("input",speedTest.displayWPM);
tryAgainBtn.addEventListener("click",speedTest.resetGame);

module.exports={displayPara,paragraphs};
